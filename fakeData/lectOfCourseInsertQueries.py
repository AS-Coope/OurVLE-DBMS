...
queryInserter = open('insertLectOfCourse.sql', 'w+')

# opening the csv file to read from it
with open('lecturer_courses.csv', mode = "r") as csvFile:
    lectOfCourseCsvFile = csv.reader(csvFile)

    # Ensuring no null values are entered in the Profession column
    
    for line in lectOfCourseCsvFile:
        #print(line)
        eachLineList.append(line)
    

# Inserting the queries into the sql file
value = "INSERT INTO LectOfCourse VALUES"
