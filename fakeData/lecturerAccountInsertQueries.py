...
queryInserter = open('insertLecturerAccount.sql', 'w+')

# opening the csv file to read from it
with open('lecturer_data.csv', mode = "r") as csvFile:
    lecturerAccountCsvFile = csv.reader(csvFile)

    # Ensuring no null values are entered in the Profession column
    
    for line in lecturerAccountCsvFile:
        #print(line)
        eachLineList.append(line)
    

# Inserting the queries into the sql file
value = "INSERT INTO LecturerAccount VALUES"
