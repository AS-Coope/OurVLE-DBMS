...
queryInserter = open('insertDepartment.sql', 'w+')

# opening the csv file to read from it
with open('Department_data.csv', mode = "r") as csvFile:
    departmentCsvFile = csv.reader(csvFile)

    # Ensuring no null values are entered in the Profession column
    
    for line in departmentCsvFile:
        print(line)
