...
queryInserter = open('insertCourseInDept.sql', 'w+')

# opening the csv file to read from it
with open('course_data.csv', mode = "r") as csvFile:
    courseInDeptCsvFile = csv.reader(csvFile)
    
    for line in courseInDeptCsvFile:
        #print(line)
        eachLineList.append(line)
    

# Inserting the queries into the sql file
value = "INSERT INTO CourseInDept VALUES"
...
for line in eachLineList[1:]:
    ...
    if currLine != 248:
        ...
        value = "("+ '"' + line[0].strip() + '"' + ',' + '"' + line[2].strip() + '"' + "),\n" 
        #print(value)
        queryInserter.write(value)
    else:
