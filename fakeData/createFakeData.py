...
""" Adding student data to variables to popluate csv file later """
""" 'i' in this case would represnt a full name """
#makes sure each course has a lecturer
lecCCount = 0
for i in lFuName:
...
for i in lFuName:
    ...
    department = random.choice(dID)
    lDeptID.append(department)
    lcount = lcount + 1
   
#Create lecturer data csv file with courses listed individually
with open('lecturer_data.csv', 'w',newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['LecturerID', 'AccountNumber', 'AccountType', 'FirstName','MiddleName', 'LastName', 'Email','Password','DepartmentID','Course1','Course2','Course3','Course4','Course5' ])
    for i in range(len(lFuName)):
        if len(lCourses[i]) == 1:
            writer.writerow([lIDs[i],lAccNos[i],lAccTypes[i],lfirstNames[i],lmiddleNames[i],llastNames[i],lemails[i],lPasswords[i], lDeptID[i], lCourses[i][0]])
        if len(lCourses[i]) == 2:
            writer.writerow([lIDs[i],lAccNos[i],lAccTypes[i],lfirstNames[i],lmiddleNames[i],llastNames[i],lemails[i],lPasswords[i], lDeptID[i], lCourses[i][0],lCourses[i][1]])
        if len(lCourses[i]) == 3:
            writer.writerow([lIDs[i],lAccNos[i],lAccTypes[i],lfirstNames[i],lmiddleNames[i],llastNames[i],lemails[i],lPasswords[i], lDeptID[i], lCourses[i][0],lCourses[i][1],lCourses[i][2]])
        if len(lCourses[i]) == 4:
            writer.writerow([lIDs[i],lAccNos[i],lAccTypes[i],lfirstNames[i],lmiddleNames[i],llastNames[i],lemails[i],lPasswords[i], lDeptID[i], lCourses[i][0],lCourses[i][1],lCourses[i][2],lCourses[i][3]])
        if len(lCourses[i]) == 5:
            writer.writerow([lIDs[i],lAccNos[i],lAccTypes[i],lfirstNames[i],lmiddleNames[i],llastNames[i],lemails[i],lPasswords[i], lDeptID[i], lCourses[i][0],lCourses[i][1],lCourses[i][2],lCourses[i][3],lCourses[i][4]])


#Create lecturer data csv file with courses in a single list
with open('lecturer_data2.csv', 'w',newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['LecturerID', 'AccountNumber', 'AccountType', 'FirstName','MiddleName', 'LastName', 'Email','Password','DepartmentID','CoursesTaught'])
